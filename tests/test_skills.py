...
def test_parse_skill_file_unknown_frontmatter_ignored(tmp_path):
    """Unknown frontmatter keys (e.g. legacy 'context: fork') are silently ignored."""
    fork_md = tmp_path / "fork.md"
    fork_md.write_text("---\nname: fork-task\ndescription: test\ncontext: fork\n---\nbody\n")
    skill = _parse_skill_file(fork_md)
    assert skill is not None
    assert skill.name == "fork-task"
